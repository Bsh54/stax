...
/// Supply the vault's stablecoin into a Kamino reserve to earn yield.
///
/// This is the yield leg: the stablecoin borrowed against the stock (see
/// `borrow_from_kamino`) is supplied to a Kamino lending reserve, which mints
/// collateral (cTokens) back to the vault and accrues supply interest. The net
/// spread between this yield and the borrow cost is what makes the deposited
/// stock productive. Signed by the vault PDA.
///
/// The caller must prepend Kamino's `refresh_reserve` instruction. klend account
/// order mirrors `deposit_reserve_liquidity`.
#[derive(Accounts)]
pub struct SupplyToKamino<'info> {
    ...
    /// Vault-owned stablecoin source; the CPI's `user_source_liquidity`.
    #[account(mut, token::authority = vault)]
    pub usdc_vault: InterfaceAccount<'info, TokenAccount>,

    /// Vault-owned account that receives the reserve collateral (cTokens);
    /// the CPI's `user_destination_collateral`.
    #[account(mut, token::authority = vault)]
    pub ctoken_vault: InterfaceAccount<'info, TokenAccount>,

    // --- klend accounts (validated by Kamino) ---
    /// CHECK: klend reserve to supply into; validated by klend.
    #[account(mut)]
...
pub fn handle_supply_to_kamino(ctx: Context<SupplyToKamino>, amount: u64) -> Result<()> {
    ...
    let mut data = discriminator::DEPOSIT_RESERVE_LIQUIDITY.to_vec();
    ...
    let metas = vec![
        ...
        AccountMeta::new(a.ctoken_vault.key(), false), // user_destination_collateral
    ...
    msg!("Supplied {} stablecoin to Kamino for yield", amount);
    Ok(())
}
